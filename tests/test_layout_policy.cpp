...
AppearanceConfig make_appearance(uint32_t padding, uint32_t border, uint32_t bar_height = 0)
{
    AppearanceConfig appearance;
    appearance.padding = padding;
    appearance.border_width = border;
    appearance.status_bar_height = bar_height;
    return appearance;
}
...
{
    AppearanceConfig appearance = make_appearance(10, 2, 20);
    Geometry area{ 0, 0, 200, 100 };

    auto slots = layout_policy::calculate_slots(1, area, appearance, false);
...
TEST_CASE("Layout policy splits two windows side by side", "[layout][policy]")
{
    ...
    auto slots = layout_policy::calculate_slots(2, area, appearance, false);
...
TEST_CASE("Layout policy accounts for internal bar in stack layout", "[layout][policy]")
{
    AppearanceConfig appearance = make_appearance(10, 2, 20);
    Geometry area{ 0, 0, 300, 200 };

    auto slots = layout_policy::calculate_slots(3, area, appearance, true);

    REQUIRE(slots.size() == 3);
    REQUIRE(slots[0].x == 12);
    REQUIRE(slots[0].y == 32);
    REQUIRE(slots[0].width == 131);
    REQUIRE(slots[0].height == 156);
    REQUIRE(slots[1].x == 157);
    REQUIRE(slots[1].y == 32);
    REQUIRE(slots[1].width == 131);
    REQUIRE(slots[1].height == 71);
    REQUIRE(slots[2].x == 157);
    REQUIRE(slots[2].y == 117);
    REQUIRE(slots[2].width == 131);
    REQUIRE(slots[2].height == 71);
}
...
TEST_CASE("Layout policy chooses nearest drop target", "[layout][policy]")
{
    ...
    size_t left_idx = layout_policy::drop_target_index(2, area, appearance, false, 20, 20);
    size_t right_idx = layout_policy::drop_target_index(2, area, appearance, false, 150, 20);
