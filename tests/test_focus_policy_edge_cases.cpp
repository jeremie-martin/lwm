...
TEST_CASE("Sticky tiled on other workspace loses to non-sticky floating on current workspace", "[focus][policy][edge]")
{
    ...
    REQUIRE(selection);
    REQUIRE(selection->window == 0x9000);
    REQUIRE(selection->is_floating);
}

TEST_CASE("Sticky tiled on other workspace wins when no current workspace candidates", "[focus][policy][edge]")
{
    Workspace ws = make_workspace({}, XCB_NONE);
