...
**Synthetic event**: An event generated by X server (or window manager) rather than by natural X11 events. For example, a synthetic ConfigureNotify is sent by the WM to inform a client of its geometry after layout changes.

**Intentional no-op**: Graceful early return with no error logging for invalid state or unsupported operations.

**Best-effort**: A value that reflects a known-good state at the time it was set, but may become inconsistent if the underlying state changes. Callers must validate before using it.
...
- modal ⇒ above: When set_window_modal(enabled) is called, it calls set_window_above(enabled). When set_window_modal(false) is called, it calls set_window_above(false). The coupling is one-way from modal to above: set_window_above() can be called independently without affecting modal state.
...
- May become stale only in edge cases:
...
**Bounds checking**: All array/vector access is validated before use
- Monitor indices are checked against `monitors_.size()`
- Workspace indices are clamped to valid range
...
- Does NOT flush connection (caller is responsible for flushing)
...
- Desktop windows: LWM never calls hide_window/show_window() on them, so hidden flag is never modified. Desktop windows are always positioned as background windows below all others.
...
- **VISIBLE** is a logical category defined by `!iconic && !hidden`. This includes windows that may have fullscreen, maximized, or other state modifiers. Fullscreen windows with `hidden=true` (e.g., on non-current workspace) are NOT in the VISIBLE category.
...
**Note**: LWM restores focus only when the removed window was on the **current workspace** of its monitor AND that monitor is the currently focused monitor. The workspace.focused_window update sets it to `workspace.windows.back()` (most recent window) or XCB_NONE if empty.
...
**Sticky Window Behavior Summary**:

**Visibility**:
- Sticky windows appear in exactly one workspace.windows vector
- They are visible on all workspaces via workspace tiling algorithm and layout filtering
- hide_window() returns early for sticky windows, so they are never moved off-screen by workspace switches

**Iconification**:
- When a sticky window is iconified, iconic flag is set
- hide_window() returns early for sticky windows (doesn't set hidden=true or move off-screen)
- Iconic sticky windows have iconic=true but hidden=false (they remain at their on-screen position while iconic flag is set)
- Iconic sticky windows are not focusable while iconic; focusing them triggers deiconification first via focus_window()
...
| Expose | handle_expose | LWM redraws status bars. Checks e.count != 0 to filter redundant events (only redraws on last one). Finds which monitor's bar window received the expose and calls bar_->update() with current window information. Only acts if bar_ is enabled. |
...
**XCB_FOCUS_IN**: LWM does not handle focus-in events.
- **Rationale**: Focus is managed proactively by LWM through SetInputFocus calls. Reactive focus management (responding to FOCUS_IN events) would cause focus conflicts and race conditions. LWM's proactive focus model ensures consistent focus state without needing to track FOCUS_IN events.

**XCB_FOCUS_OUT**: LWM does not handle focus-out events.
- **Rationale**: LWM always knows which window should be focused through its own state tracking. Tracking FOCUS_OUT events would be redundant and could introduce inconsistencies between X server focus state and LWM's internal state.

**XCB_LEAVE_NOTIFY**: LWM extracts the timestamp from LeaveNotify events (via extract_event_time()) but does not handle them.
- **Rationale**: LeaveNotify is only used for timestamp tracking for focus stealing prevention. The actual focus behavior is managed through EnterNotify events and explicit focus operations. LeaveNotify itself does not trigger any window manager actions.

**Synthetic event**: An event generated by X server (or window manager) rather than by natural X11 events. For example, a synthetic ConfigureNotify is sent by the WM to inform a client of its geometry after layout changes.

**MRU (Most Recently Used)**: Reverse iteration through vectors (newest items at end) provides MRU ordering for focus restoration and floating window stacking.

**Best-effort**: A value that reflects a known-good state at the time it was set, but may become inconsistent if the underlying state changes. For example, workspace.focused_window points to the last-focused tiled window, but if that window moves to a different workspace, the value becomes stale. Callers must validate the value before using it (e.g., check that the window still exists in the workspace).
...
**Empty workspace behavior**: When switching to an empty workspace (no windows) or when the last window on a workspace is removed, `focus_or_fallback()` finds no candidates and clears focus (sets `active_window_ = XCB_NONE`).
...
- Callers validate existence and eligibility before using it
  - For example, select_focus_candidate() validates workspace.focused_window before using it
...
- When _NET_WM_USER_TIME changes on user_time_window:
     - LWM finds parent Client that owns this user_time_window
     - LWM updates parent Client.user_time with the new value
...
**See [Workspace Management → Sticky Windows](#sticky-windows) for complete sticky window behavior documentation.**
...
**See [Window State Machine → Maximized Transition](#maximized-transition) for complete maximized behavior including:**
